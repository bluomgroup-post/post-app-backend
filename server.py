...
def send_otp_sms(phone, code):
    ...
    try:
        ...
        twilio.messages.create(
            body=f"Your POST verification code: {code}\nValid for 10 minutes.",
            from_=TWILIO_PHONE,
            to=phone
        )
        logging.info(f"OTP SMS sent to {phone}")
        return True
    except Exception as e:
...
class PhoneSignupIn(BaseModel):
    phone: str; password: str; name: str
...
async def phone_signup(p: PhoneSignupIn):
    existing = await db.users.find_one({"phone": p.phone})
    if existing and existing.get("is_verified"):
    ...
    colors = ["#FFD600","#00C853","#FF1744","#2979FF"]
    doc = {
        "id": uid, "phone": p.phone, "email": f"{p.phone}@phone.post",
        "name": p.name,
        "handle": f"@{p.name.lower().replace(' ','_')}",
        "password_hash": hashpw(p.password), "is_verified": False,
        "otp_hash": hashpw(code), "otp_expires_at": now() + timedelta(minutes=10),
    ...
    return {"message": "OTP sent via SMS", "demo_otp": code if not sms_sent else None}

@api.post("/auth/phone-verify-otp")
async def phone_verify_otp(p: PhoneOtpIn):
